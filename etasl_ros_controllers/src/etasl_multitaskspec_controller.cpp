 ...
 * Very simple copy of EtaslController with support for a list of
 * specification files to be loaded from the parameter server and
 * combined.
 */
...
#include <etasl_ros_controllers/etasl_multitaskspec_controller.h>

namespace etasl_ros_controllers
{
bool EtaslMultitaskspecController::init(hardware_interface::RobotHW* robot_hardware, ros::NodeHandle& node_handle)
{
  position_joint_interface_ = robot_hardware->get<hardware_interface::PositionJointInterface>();
  if (position_joint_interface_ == nullptr)
  {
    ROS_ERROR("EtaslMultitaskspecController: Error getting position joint interface from hardware!");
    return false;
  }

  if (!node_handle.getParam("joints", joint_names_))
  {
    ROS_ERROR("EtaslMultitaskspecController: Could not parse joint names");
  }
  ...
  for (size_t i = 0; i < n_joints_; ++i)
  {
    ...
    {
      ROS_ERROR_STREAM("EtaslMultitaskspecController: Exception getting joint handles: " << e.what());
      return false;
    }
  }
...
void EtaslMultitaskspecController::starting(const ros::Time& /* time */)
{
  for (size_t i = 0; i < n_joints_; ++i)
  {
  ...
  {
    ROS_ERROR_STREAM("EtaslMultitaskspecController: Could not initialize the eTaSl solver");
  }
}

void EtaslMultitaskspecController::update(const ros::Time& /*time*/, const ros::Duration& period)
{
...
bool EtaslMultitaskspecController::configureInput(ros::NodeHandle& node_handle)
{
  if (node_handle.getParam("input/names", input_names_) && node_handle.getParam("input/types", input_types_))
  {
    ...
    {
      ROS_ERROR_STREAM("EtaslMultitaskspecController: The number of input names and input types must be the same");
      return false;
    }
    n_inputs_ = input_names_.size();
    ROS_INFO_STREAM("EtaslMultitaskspecController: Found " << n_inputs_ << " input channels");

    for (size_t i = 0; i < n_inputs_; ++i)
    {
      if (input_types_[i] == "Scalar")
      {
        ROS_INFO_STREAM("EtaslMultitaskspecController: Adding input channel \"" << input_names_[i] << "\" of type \"Scalar\"");
      ...
      {
        ROS_INFO_STREAM("EtaslMultitaskspecController: Adding input channel \"" << input_names_[i] << "\" of type \"Vector\"");
      ...
      {
        ROS_INFO_STREAM("EtaslMultitaskspecController: Adding input channel \"" << input_names_[i] << "\" of type \"Rotation\"");
      ...
      {
        ROS_INFO_STREAM("EtaslMultitaskspecController: Adding input channel \"" << input_names_[i] << "\" of type \"Frame\"");
      ...
      {
        ROS_INFO_STREAM("EtaslMultitaskspecController: Adding input channel \"" << input_names_[i] << "\" of type \"Twist\"");
      ...
      {
        ROS_ERROR_STREAM("EtaslMultitaskspecController: Input channel type \"" << input_types_[i] << "\" is not supported");
        return false;
      }
    }
  }
  ...
  {
    ROS_INFO("EtaslMultitaskspecController: Could not find input channels on parameter server");
  }
  return true;
}

void EtaslMultitaskspecController::getInput()
{
  // Read inputs
...
bool EtaslMultitaskspecController::configureOutput(ros::NodeHandle& node_handle)
{
  if (node_handle.getParam("output/names", output_names_) && node_handle.getParam("output/types", output_types_))
  {
    ...
    {
      ROS_ERROR_STREAM("EtaslMultitaskspecController: The number of output names and output types must be the same");
      return false;
    }

    n_outputs_ = output_names_.size();
    ROS_INFO_STREAM("EtaslMultitaskspecController: Found " << n_outputs_ << " output channels");

    for (size_t i = 0; i < n_outputs_; ++i)
    {
      if (output_types_[i] == "Scalar")
      {
        ROS_INFO_STREAM("EtaslMultitaskspecController: Adding output channel \"" << output_names_[i] << "\" of type \"Scalar\"");
      ...
      {
        ROS_INFO_STREAM("EtaslMultitaskspecController: Adding output channel \"" << output_names_[i] << "\" of type \"Vector\"");
      ...
      {
        ROS_INFO_STREAM("EtaslMultitaskspecController: Adding output channel \"" << output_names_[i] << "\" of type \"Rotation\"");
      ...
      {
        ROS_INFO_STREAM("EtaslMultitaskspecController: Adding output channel \"" << output_names_[i] << "\" of type \"Frame\"");
      ...
      {
        ROS_INFO_STREAM("EtaslMultitaskspecController: Adding output channel \"" << output_names_[i] << "\" of type \"Twist\"");
      ...
      {
        ROS_ERROR_STREAM("EtaslMultitaskspecController: Output channel type \"" << output_types_[i] << "\" is not supported");
        return false;
      }
    }
  }
  ...
  {
    ROS_INFO("EtaslMultitaskspecController: Could not find output channels on parameter server");
  }
}

void EtaslMultitaskspecController::setOutput()
{
...
PLUGINLIB_EXPORT_CLASS(etasl_ros_controllers::EtaslMultitaskspecController, controller_interface::ControllerBase)
